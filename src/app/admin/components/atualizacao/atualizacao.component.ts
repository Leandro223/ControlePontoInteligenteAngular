...
export class AtualizacaoComponent implements OnInit {
  ...
  obterLancamento(dados: any): Lancamento {
    let data = moment(dados.data);

    let horas = dados.horas;
    let minutos = dados.minutos;
    let tipo = dados.tipo;

    if (!dados.data) {
      data = moment(); // preenche com a data atual se não foi preenchido
    }

    if (!dados.horas) {
      horas = this.form.get('horas').value; // preenche com o valor atual se não foi preenchido
    }

    if (!dados.minutos) {
      minutos = this.form.get('minutos').value; // preenche com o valor atual se não foi preenchido
    }

    if (!dados.tipo) {
      tipo = this.form.get('tipo').value; // preenche com o valor atual se não foi preenchido
    }
